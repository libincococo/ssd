...
from preprocessing.ssd_preprocessing import *
...
def input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        ...
        image_preprocessing_fn = \
            lambda image_, labels_, bboxes_: preprocess_image(image_,
                                                              labels_,
                                                              bboxes_,
                                                              out_shape,
        ...
        anchor_encoder_fn = lambda glabels_, gbboxes_: anchor_encoder_decoder.encode_all_anchors(glabels_,
                                                                                                 gbboxes_,
                                                                                                 all_anchors,
        ...
        image, _, shape, loc_targets, cls_targets, match_scores = \
            dataset_common.slim_get_batch(FLAGS.num_classes,
                                          batch_size,
        ...
        global_anchor_info = {'decode_fn': lambda pred:
        anchor_encoder_decoder.decode_all_anchors(pred,
                                                  num_anchors_per_layer),
                                                  'num_anchors_per_layer': num_anchors_per_layer,
                                                  'all_num_anchors_depth': all_num_anchors_depth}

        return image, {'shape': shape, 'loc_targets': loc_targets, 'cls_targets': cls_targets,
    ...
    with tf.device('/cpu:0'):
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                predictions_for_bg = tf.nn.softmax(tf.reshape(cls_pred, [tf.shape(features)[0],
                                                                         -1,
...
def main(_):
    ...
    replicate_ssd_model_fn = tf.contrib.estimator.replicate_model_fn(ssd_model_fn,
