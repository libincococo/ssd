...
def do_dual_max_match(overlap_matrix, low_thres, high_thres, ignore_between=True, gt_max_first=True):
    ...
    with tf.name_scope('dual_max_match', [overlap_matrix]):
        ...
        anchors_to_gt_mask = tf.one_hot(
            tf.clip_by_value(match_indices, -1, tf.cast(tf.shape(overlap_matrix)[0], tf.int64)),
        ...
        if gt_max_first:
            ...
            left_gt_to_anchors_mask = tf.one_hot(gt_to_anchors, tf.shape(overlap_matrix)[1], on_value=1, off_value=0,
            ...
            left_gt_to_anchors_mask = tf.cast(
                tf.logical_and(tf.reduce_max(anchors_to_gt_mask, axis=1, keep_dims=True) < 1,
        ...
        selected_scores = tf.gather_nd(overlap_matrix,
